...
- 堆：存储动态分配的内存，通常用于需要在多个函数间共享的对象。堆上的数据需要手动管理或由 Go的垃圾回收机制负责清理。
- 栈：用于函数调用时

![707](./go.assets/img/go_memory_layout.svg)
- 代码段 `.text`：存放所有函数编译后的机器指令，**只读**，运行时不可写入。函数值（`func` 类型变量）本质上是一个指向代码段中某地址的指针。

- 只读数据段 `.rodata`：存放**常量**的具体值和**字符串字面量**的字节内容。`const Pi = 3.14` 这类简单数值常量通常会被编译器**内联**到使用处（不占独立地址），但需要取地址的常量或字符串内容则存在此段，运行时写入触发 panic。

- 数据段 `.data`：存放**有初始值的包级变量**（`var` + 非零初始值）。它们在程序整个生命周期内持续存在，可读可写。

- BSS 段：存放**零值的包级变量**（`var x int` 未显式赋值）。不占可执行文件体积，程序启动时由操作系统清零初始化。

- 堆 Heap：`new()`、`make()`、`&T{}`、以及编译器**逃逸分析**判定生命周期超出函数范围的变量，都分配在堆上。由 Go 运行时的 GC（三色标记+写屏障）自动管理回收。

- 栈 Stack：每个 goroutine 有**独立的栈**，初始 2–8 KB，可按需增长（Go 使用分段栈/连续栈复制）。函数局部变量、参数、返回值分配于此，函数返回即自动释放，无 GC 参与。

###### 常量
`const`：只读数据段（.rodata）
![707](./go.assets/img/const_rodata.svg)
`const` 是纯编译期常量，数值类型的 `const` 会被直接内联到使用它的指令里（根本没有地址），字符串类型的 `const` 字节数据存在 `.rodata`。
```go
...
var name = "go" // .data 段：string header（ptr+len）在 .data，字节 "go" 本身在 .rodata
```
...
##### bytes
`bytes` 包是 Go 标准库中专门用于操作 **字节切片（`[]byte`）** 的工具包，它提供了大量与 `strings` 包对等的函数，但处理对象是字节而非字符串。因为字节切片是可变、高效且与底层数据紧密关联的，`bytes` 包在涉及 I/O、网络、二进制协议、缓存等场景中极为常用。
...
fmt.Printf("转大写%s, 转小写%s\n", string(upper), string(lower)) // 转大写ABCD, 转小写abce
```
...
// 它会分配一个底层数组，并返回一个包含 5 个元素（都初new(始化为零值）的切片，
...
```
注意：切片指针 `(*pSlice)[1]` 不能简写为 `pSlice[1]`，必须先\*解引用得到 `[]int`，再下标访问 
...
**new 与 make函数区别**
- `func new(Type) *Type`，new (T)：为每个新的类型 T 分配一片内存，初始化为 0 并且返回类型为 * T 的内存地址：这种方法 返回一个指向类型为 T，值为 0 的地址的指针，它适用于值类型如数组和结构体它相当于 &T{}。
- make(T) 返回一个类型为 T 的初始值，它只适用于 3 种内建的引用类型：切片、map 和 channel

![443](./go.assets/img/new_and_make.png)
- `new([]int)`，切片未初始化，值为`nil`。
- `make([]int, 0)` ，切片已经被初始化，但是指向一个空的数组。
...
```
其中cap表示map的**初始容量提示（hint）**，告诉运行时：“预计这个 map 大概要存放 1000 个左右的键值对，请提前分配好足够的内存，避免后面频繁扩容”。只是一个**建议值**，不是严格的容量上限。存超过 1000 个元素，map 会自动扩容，不会有任何报错，也不会受这个 `1000` 的限制。
...
map 对于不存的键其返回值是对应类型的零值，并且在访问 map 的时候其实有两个返回值，第一个返回值对应类型的值，第二个返回值一个布尔值，表示键是否存在。
