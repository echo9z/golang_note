...
## make与new函数参数与返回值问题
make 内置函数分配并初始化 slice、map 或 chan（仅）类型的对象。与 new 一样，第一个参数是类型，而不是值。与 new 不同，make 的返回类型与其参数的类型相同，而不是指向它的指针。这里而不是指向它的指针是什么意思？

“返回类型与其参数的类型相同，而不是指向它的指针”这句话指的是==**返回的具体数据形态**==。简单来说：**`make` 直接返回初始化后的“值”本身，而 `new` 返回的是指向该类型的“内存地址（指针）”**。 

1. 使用 `new`（返回指针）
`new(T)` 分配了 `T` 类型的零值内存，并**返回指向它的指针**（类型为 `*T`）。
```go
s := new([]int)    // s 的类型是 *[]int （指向切片的指针）
m := new(map[int]string) // m 的类型是 *map[int]string （指向字典的指针）
```

因为得到的是指针，如果你想往里面存数据或读取数据，必须先对其进行解引用（例如 `*s`）。

2. 使用 `make`（返回类型本身）
`make(T, ...)` 分配并**初始化**了特定的引用类型，直接**返回类型 `T` 本身**，而不是指针。 
```go
s := make([]int, 5)    // s 的类型是 []int （切片类型本身）
m := make(map[int]string) // m 的类型是 map[int]string （字典类型本身）
```

因为 `make` 返回的就是切片或字典本身，可以直接拿来赋值和操作（如直接 `s[0] = 1`），不需要通过指针解引用。

## 在go中struct{}{}是什么意思
`struct{}{}` 由两部分组成，是一个**空结构体的字面量实例**：

|部分|含义|
|---|---|
|`struct{}`|**类型** —— 一个没有任何字段的结构体类型（空结构体）|
|`{}`|**字面量** —— 用复合字面量语法实例化这个类型，得到一个值|

所以 `struct{}{}` 表示「创建一个空结构体类型的值」。可以类比 `Point{1, 2}` 理解：`Point` 是类型，`{1, 2}` 是初始化，只是这里类型恰好是匿名的、且没有任何字段。

### 关键特性

`struct{}` 最特殊的一点是**占用 0 字节内存**（确切地说，`unsafe.Sizeof(struct{}{}) == 0`）。因此它被广泛用于「只需要占位、不需要存数据」的场景。

### 三大典型用途

#### 1. 实现集合（Set）

`map` 的 value 用 `struct{}` 代替 `bool`，省内存：

```go
set := map[string]struct{}{}
set["a"] = struct{}{}        // 加入集合
_, exists := set["a"]        // 判断是否存在
delete(set, "a")             // 删除
```

#### 2. 信号通道（Signal channel）

只用来通知「事情发生了」，不需要传递任何数据：
...
go func() {
    // 干活...
    close(done)              // 通过 close 发送信号
}()

<-done                       // 阻塞，直到 close
```

对比 `chan bool`，`chan struct{}` 在语义上更明确：**这个通道不携带数据，只用于同步**。

#### 3. 方法接收者

用空结构体作为接收者类型，表示「这个类型本身只是方法的容器，不存任何状态」：
...
非常准确且形象地揭示了计算机编程（特别是 C/C++ 等语言）中**指针** 与 **指针变量**的核心本质。
为了让你更容易理解，我们可以用一个**“酒店与房间”的比喻**，并把它拆分成两个层次来解读：
...
### 二、分句详细拆解
#### 1. 第一句：“任何程序数据载入内存后，在内存都有他们的地址，这就是指针。”
- **原理**：计算机的内存是由亿万个微小的存储单元组成的。程序运行起来后，所有的变量、函数、代码指令都会被加载进内存。为了找到这些数据，内存给每一个位置都编了号，这个编号就是**内存地址**（如 0x7ffd51b689ac）。
- **为什么叫指针？**：因为这个“内存地址”就像一只手指，**指明了数据到底藏在内存的什么地方**。因此，在概念上，**内存地址本身 = 指针**。

#### 2. 第二句：“而为了保存一个数据在内存中的地址，我们就需要指针变量。”
- **原理**：
    - 普通变量（比如 int a = 10;）是用来保存**普通数据**（如数字 10）的。
    - 但有时我们需要在程序里把“某个数据的地址（房间号）”记录下来，以便后面直接通过地址去访问或修改这个数据。
    - 既然地址也是一串数字，我们就需要一种专门的变量来存储这个“地址数字”，这种专门用来存地址的变量，就叫**指针变量**。


### 三、明确两个概念的区别（常考点/易混淆点）

在日常交流中，程序员常常把“指针变量”简称为“指针”，但从严格的逻辑上讲，它们是有区别的：

|   |   |   |   |
|---|---|---|---|
|概念|含义|比喻|代码中的体现|
|**指针 (Pointer)**|是一个**地址数值**（客观存在的内存位置）|房间号（如 301）|&a（取变量 a 的地址）|
|**指针变量 (Pointer Variable)**|是一个**变量容器**（用来盛放地址的变量）|写着“301”的记事纸|int *p;（声明变量 p 专门存地址）|
...
int main() {
    int a = 10;      // 1. 数据 10 被存入内存。
                     //    假设它在内存中的地址是 0x2000。
                     //    【0x2000 这个地址本身，就是“指针”】

    int *p = &a;     // 2. 取出 a 的地址 (&a)，赋值给 p。
                     //    【p 就是“指针变量”】，它里面存的值就是 0x2000。

    printf("a 的值是: %d\n", a);   // 输出数据：10
    printf("a 的地址是: %p\n", &a); // 输出指针（地址）：例如 0x7ffd...
    printf("p 保存的值: %p\n", p);  // 输出指针变量里存的内容：和 &a 一模一样

    return 0;
}
```

这句话的核心思想就是：  **数据在内存里都有“门牌号”（门牌号就是指针），而用来记下这个“门牌号”的小本本，就是指针变量。**